        ...
        public:
            ...
            static TypeId GetTypeId() {
                static TypeId tid = TypeId("ns3::aodv::DeferredRouteOutputTag")
                        .SetParent<Tag>()
                        .SetGroupName("Aodv")
                        .AddConstructor<DeferredRouteOutputTag>();   //SetGroupName is new CNLAB
                return tid;
            }
...
//CNLAB
                ...
                if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
                    if (dst == iface.GetBroadcast() || dst.IsBroadcast()) {
                        ...
                        if (lcb.IsNull() == false) {
                            ...
                            NS_LOG_ERROR(
                                    "Unable to deliver packet locally due to null callback " << p->GetUid() << " from "
                                                                                             << origin);
        ...
        RoutingProtocol::SetIpv4(Ptr <Ipv4> ipv4) {
            ...
            NS_ASSERT(m_lo != 0);
            // Remember lo route
            RoutingTableEntry rt(/*device=*/ m_lo, /*dst=*/ Ipv4Address::GetLoopback(), /*know seqno=*/ true, /*seqno=*/
                                             0,
        ...
        RoutingProtocol::LoopbackRoute(const Ipv4Header &hdr, Ptr <NetDevice> oif) const {
            ...
            //
            std::map < Ptr < Socket > , Ipv4InterfaceAddress > ::const_iterator
            ...
            if (!m_routingTable.LookupRoute(rrepHeader.GetDst(), toNeighbor)) {
                ...
                if (EnableWrmAttack && wrmDst == FirstEndOfWormTunnel) {
                    ...
                    toNeighbor.SetOutputDevice(
                            m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(SecondEndOfWormTunnel)));
                    toNeighbor.SetInterface(
                            m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(SecondEndOfWormTunnel), 0));
                    toNeighbor.SetHop(1);
                    toNeighbor.SetNextHop(rrepHeader.GetDst());
                } else if (EnableWrmAttack && wrmDst == SecondEndOfWormTunnel) {
                    // cout<<"RREP Helper Contains Second End P2P interface Address";
                    toNeighbor.SetOutputDevice(
                            m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(FirstEndOfWormTunnel)));
                    toNeighbor.SetInterface(
                            m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(FirstEndOfWormTunnel), 0));
                    toNeighbor.SetHop(1);
                    toNeighbor.SetNextHop(rrepHeader.GetDst());
